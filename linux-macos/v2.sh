...
function has_remote_branch {
  local repo_path="$1"  # Git 项目路径
  local branch_name="$2"  # 远程分支名（如 origin/feature/courselive）

  # 构造完整的远程分支引用路径
  local full_ref="refs/remotes/$branch_name"

  # 检查引用是否存在
  if git -C "$repo_path" show-ref --verify --quiet "$full_ref"; then
    return 0  # 存在
  else
    return 1  # 不存在
  fi
...
branch_list=(${(k)branch_counts})
branch_length=${#branch_list[@]}
echo "所有模块包含的分支($branch_length):"
printf '  %s\n' "${branch_list[@]}"
...
# 逐个判断每个模块是否存在该分支，若存在则进行切换，不存在则输出提示语句
# 切换分支
success_count=0
fail_count=0
void_branch_count=0
...
for module in "${git_modules[@]}"; do
  if has_remote_branch "$module" "$selected_branch"; then
    echo "$module：分支 $selected_branch 存在"
    selected_final_branch=${selected_branch#origin/}
    echo -n "正在切换 $module ... "
    if output=$(git -C "$module" -c credential.helper= -c core.quotepath=false -c log.showSignature=false checkout "$selected_final_branch" -- 2>&1); then
      echo "✓"
      ((success_count++))
    else
      echo "✗"
    ...
    echo "分支 $selected_branch 不存在 $module"
    ((void_branch_count++))
  fi
done
...
echo "空分支: $void_branch_count 个模块"
